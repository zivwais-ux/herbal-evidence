...
| D-015 | 2026-09-23 | Phase 1 schema: `requests` has no client UPDATE policy at all (not even for the owner) — every status transition and edit goes through the backend with the service role, which also enforces the `enforce_request_status_transition()` trigger. Researchers/admins get read access to `requests`/`profiles`/`published_results` via RLS, but `ai_drafts`, `literature_sources`, `researcher_reviews`, and `research_jobs` have **no** policies for any authenticated role, per D-011 — even staff reads them only through the backend's own authorization check. | Defense in depth: a leaked researcher JWT alone can't read AI drafts or job internals | default |
